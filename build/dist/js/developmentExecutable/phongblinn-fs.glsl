...
//16. dia phongblinn shader maxblinn alapjan
...
vec3 shade(vec3 normal, vec3 lightDir, vec3 viewDir, vec3 powerDensity, vec3 materialColor, vec3 specularColor, float shininess) {
  ...
  return
    materialColor * cosa * powerDensity + specularColor * specular * powerDensity;
}
...
void main(void) {
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    vec3 shadeOut = shade(worldNormal.xyz,
      ...
      texture(material.colorTexture, tex.xy/tex.w).rgb, material.specularColor, material.shininess);
    if (shadeOut.x < 0.0) {
      shadeOut.x = 0.0;
    } if (shadeOut.y < 0.0) {
      shadeOut.y = 0.0;
