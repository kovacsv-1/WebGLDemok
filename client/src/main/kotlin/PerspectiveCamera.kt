...
class PerspectiveCamera(vararg programs : Program) : UniformProvider("camera") {
  ...
  fun move(dt : Float, keysPressed : Set<String>, avatar : GameObject) {
    /*if(isDragging) {
    ...
    //set lookat to above avatar
    //set position to behind/above avatar

    val behind = Vec3(-100.0f * sin(avatar.yaw), 50.0f, -100.0f * cos(avatar.yaw)) + avatar.position
    position.set(behind)
    pitch = -3.141592f * 0.1f
    yaw = 3.141592f + avatar.yaw
    if (yaw < -3.141592f) {yaw += 2.0f * 3.141592f}
    if (yaw > 3.141592f) {yaw -= 2.0f * 3.141592f}
